...
from lithops.multiprocessing import Array, Barrier, RawValue
...
class KMeans:
    ...
    def initialize_workers(self):
        for worker_idx in range(self.n_workers):
            worker = Worker(worker_idx, self.n_workers, self.n_processes, self.n_clusters, self.max_iter, self.centroids_initialized, self.cluster_centers, self.clusters_totals, self.clusters_counters, self.barrier, self.has_converged)
            self.workers.append(worker)

    def fit(self, X):
        start_time = time.time()

        self.clusters_totals = [Array('d', [0 for _ in range(self.n_clusters)]) for idx in range(self.n_workers)]
        self.clusters_counters = [Array('i', [0 for _ in range(self.n_clusters)]) for idx in range(self.n_workers)]
...
class Worker:
    def __init__(self, worker_id, n_workers, n_processes, n_clusters, max_iter, centroids_initialized, cluster_centers, clusters_totals, clusters_counters, barrier, has_converged):
    ...
    def run(self):
        ...
        for iter in range(self.max_iter):
            ...
            for process_idx in range(self.n_processes):
                ...
                process = mp.Process(target=self.compute_clusters, args=(child_conn, partitioned_X[process_idx], cluster_centers_local,))
            ...
            for cluster_idx in range(self.n_clusters):
                self.clusters_counters[self.worker_id][cluster_idx] = clusters_counters_local[cluster_idx]
                self.clusters_totals[self.worker_id][cluster_idx] = clusters_totals_local[cluster_idx]
            ...
            if self.worker_id == 0:
                ...
                for cluster_idx in range(self.n_clusters):
                    ...
                    cluster_totals = 0
                    for worker_idx in range(self.n_workers):
                        cluster_counters += self.clusters_counters[worker_idx][cluster_idx]
                        cluster_totals += self.clusters_totals[worker_idx][cluster_idx]

                    if cluster_counters > 0:
                        cluster_centers_new.append(cluster_totals / cluster_counters)
                    else:
